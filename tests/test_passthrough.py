...
class TestPassthroughOption:
    ...
    def test_passthrough_single_option(self, mock_subprocess, mock_get_path, mock_validate):
        ...
        result = self.runner.invoke(app, [
            "cc", "--cpu", "PIC16F877A", "--xc8-version", "3.00",
            "--passthrough", "-mplib",
            "main.c"
        ])
    ...
    @patch('xc8_wrapper.core.get_xc8_tool_path')
    @patch('subprocess.run')
    def test_passthrough_multiple_options(self, mock_subprocess, mock_get_path, mock_validate):
    ...
    @patch('xc8_wrapper.core.get_xc8_tool_path')
    @patch('subprocess.run')
    def test_passthrough_with_quoted_values(self, mock_subprocess, mock_get_path, mock_validate):
        ...
        result = self.runner.invoke(app, [
            "cc", "--cpu", "PIC16F877A", "--xc8-version", "3.00",
            "--passthrough", '-mchecksum=0x1234 --fill="0xFF" -mserial="ABC123"',
            "main.c"
        ])
    ...
    @patch('xc8_wrapper.core.get_xc8_tool_path')
    @patch('subprocess.run')
    def test_passthrough_combined_with_regular_options(self, mock_subprocess, mock_get_path, mock_validate):
        ...
        result = self.runner.invoke(app, [
            "cc", "--cpu", "PIC16F877A", "--xc8-version", "3.00",
            "-O2", "-v",  # Regular options
            "-D", "DEBUG=1",  # Regular define
            "--passthrough", "-mplib -gdwarf-3",  # Passthrough options
            "main.c"
        ])
    ...
    def test_passthrough_invalid_syntax(self):
        ...
        result = self.runner.invoke(app, [
            "cc", "--cpu", "PIC16F877A", "--xc8-version", "3.00",
            "--passthrough", 'invalid "unclosed quote',
            "main.c"
        ])
    ...
    @patch('xc8_wrapper.core.get_xc8_tool_path')
    @patch('subprocess.run')
    def test_passthrough_empty_string(self, mock_subprocess, mock_get_path, mock_validate):
...
class TestPassthroughIntegration:
    ...
    def test_passthrough_complex_real_world_example(self):
        ...
        result = runner.invoke(app, [
            "cc", "--cpu", "PIC18F4550", "--xc8-version", "3.00",
            "-O2", "-v",  # Regular options
            "-I", "./include", "-I", "./lib",  # Include paths
            "-D", "DEBUG=1", "-D", "VERSION=100",  # Defines
            "--passthrough", passthrough_opts,  # Passthrough
            "src/main.c", "src/uart.c", "src/spi.c"  # Multiple files
        ])
    ...
    def test_passthrough_blocks_file_traversal(self, mock_get_path, mock_validate):
        ...
        for dangerous_path in dangerous_paths:
            result = self.runner.invoke(app, [
                "cc",
                "--cpu", "PIC16F876A",
                "--passthrough", f"-I {dangerous_path}",
                "test.c"
            ])
            # Should exit with error due to security validation
            assert result.exit_code != 0, f"File traversal should be blocked: {dangerous_path}"
    
    @patch('xc8_wrapper.core.validate_xc8_tool')
    @patch('xc8_wrapper.core.get_xc8_tool_path')
    @patch('subprocess.run')
    def test_passthrough_allows_legitimate_compiler_options(self, mock_subprocess, mock_get_path, mock_validate):
